...
if(isset($_GET['start'])){
	...
	$Qnews = mysql_query("SELECT * FROM tb_article WHERE art_type='$id' ORDER BY art_id DESC LIMIT $start,$limit");
...
for($i=1;$i<=$page;$i++){
	...
	echo " <a href='?id=$art_type&start=".$limit*($i-1)."&page=$i'><B>[$i]</B></A>"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 1
	} else {
	echo " <a href='?id=$art_type&start=".$limit*($i-1)."&page=$i'><B>[$i]</B></A>"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 2
	}
}
echo"</center>";
echo "<br />";
